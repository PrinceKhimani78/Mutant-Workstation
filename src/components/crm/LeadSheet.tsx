...
export function LeadSheet({
  ...
  // Inline Decision Maker State for Table Rows
  const [inlineAddLeadId, setInlineAddLeadId] = useState<string | null>(null);
  const [inlinePerson, setInlinePerson] = useState<DecisionMaker>({
    name: '',
    designation: '',
    linkedin: '',
    email: '',
  ...
  const submitInlineDecisionMaker = async (lead: any) => {
    if (!inlinePerson.name && !inlinePerson.designation && !inlinePerson.linkedin && !inlinePerson.email && !inlinePerson.phone) {
      setInlineAddLeadId(null);
      return;
    }

    const existing = parseContacts(lead.contacts);
    const updatedContacts = [...existing, { ...inlinePerson }];
    ...
    const res = await fetch(`/api/crm/leads/${lead.id}`, {
      ...
      body: JSON.stringify({
        ...
        name: lead.name || inlinePerson.name,
        email: lead.email || inlinePerson.email,
        phone: lead.phone || inlinePerson.phone,
      }),
    });

    if (res.ok) {
      setInlineAddLeadId(null);
      setInlinePerson({ name: '', designation: '', linkedin: '', email: '', phone: '' });
      onRefresh();
    } else {
      const data = await res.json();
      alert(data.error || 'Failed to add decision maker');
    }
  };
                ...
                return (
                  <tr key={lead.id} className="hover:bg-[var(--surface-muted)]/50 transition-colors align-top">
                    ...
                    <td className="p-3 space-y-2">
                      ...
                      {inlineAddLeadId === lead.id ? (
                        <div className="p-2.5 rounded-lg bg-[var(--surface)] border border-[var(--primary)]/30 space-y-2 text-xs shadow-sm mt-1.5">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-bold text-[var(--foreground)]">New Decision Maker</span>
                            <button
                              type="button"
                              onClick={() => setInlineAddLeadId(null)}
                          ...
                          <div className="grid grid-cols-2 gap-1.5">
                            <input
                              ...
                              value={inlinePerson.name}
                              onChange={(e) => setInlinePerson({ ...inlinePerson, name: e.target.value })}
                              className="input-minimal px-2 py-1 text-xs bg-white"
                              autoFocus
                            />
                            <input
                              type="text"
                              placeholder="Title (e.g. CEO, CTO)"
                              value={inlinePerson.designation || ''}
                              onChange={(e) => setInlinePerson({ ...inlinePerson, designation: e.target.value })}
                              className="input-minimal px-2 py-1 text-xs bg-white"
                            />
                          </div>
                          <div className="grid grid-cols-2 gap-1.5">
                            <input
                              type="text"
                              placeholder="LinkedIn Profile URL"
                              value={inlinePerson.linkedin || ''}
                              onChange={(e) => setInlinePerson({ ...inlinePerson, linkedin: e.target.value })}
                              className="input-minimal px-2 py-1 text-xs bg-white"
                            />
                            <input
                              type="email"
                              placeholder="Email address"
                              value={inlinePerson.email || ''}
                              onChange={(e) => setInlinePerson({ ...inlinePerson, email: e.target.value })}
                              className="input-minimal px-2 py-1 text-xs bg-white"
                            />
                          </div>
                          <div className="flex items-center justify-between gap-2 pt-0.5">
                            <input
                              type="text"
                              placeholder="Phone / WhatsApp"
                              value={inlinePerson.phone || ''}
                              onChange={(e) => setInlinePerson({ ...inlinePerson, phone: e.target.value })}
                              className="input-minimal px-2 py-1 text-xs bg-white flex-1"
                            />
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button
                                type="button"
                                onClick={() => setInlineAddLeadId(null)}
                                className="px-2 py-1 rounded text-[10px] text-[var(--muted-foreground)] hover:bg-[var(--surface-muted)]"
                              >
                                Cancel
                              </button>
                              <button
                                type="button"
                                onClick={() => submitInlineDecisionMaker(lead)}
                                className="btn btn-primary px-3 py-1 rounded text-[10px] font-semibold"
                              >
                                Save
                              </button>
                            </div>
