...
export async function POST(request: Request) {
  try {
    ...
    const [leads, projects, tasks, clients, invoices, sops] = await Promise.all([
      db.lead.findMany({ select: { name: true, company: true, budget: true, source: true, stage: { select: { name: true } } } }),
    ...
    if (lowerQuery.includes('lead') || lowerQuery.includes('pipeline') || lowerQuery.includes('sales')) {
      const pendingLeads = leads.filter(l => l.stage?.name !== 'Won' && l.stage?.name !== 'Lost');
      const totalPipeline = leads.reduce((sum, l) => sum + (l.budget || 0), 0);
      const pipelineLine = owner ? `- **Active Pipeline Value:** $${totalPipeline.toLocaleString()}\n` : '';
      responseText = `⚡ **Mutant AI Lead & Pipeline Analysis:**\n${pipelineLine}- **Active Leads:** ${pendingLeads.length} leads in qualification.\n- **Top Sources:** ${Array.from(new Set(leads.map(l => l.source))).join(', ')}.\n\n👉 **Recommended Action:** Follow up with *${leads[0]?.name || 'pending leads'}* currently in the *${leads[0]?.stage?.name || 'Proposal Sent'}* stage to close this week.`;
      ...
      if (!owner) {
        responseText = `🔒 Revenue and invoicing figures are only visible to the account owner — ask Prince directly for a financial summary.`;
        suggestedActions = ['View Clients', 'View Sales Pipeline'];
      } else {
